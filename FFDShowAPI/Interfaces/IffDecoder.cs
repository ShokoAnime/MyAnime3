...
namespace FFDShowAPI.Interfaces
{
    ...
    public interface IffDecoder
    {
        ...
        int compat_getFilterTipEx(uint filterID, uint filterPageId, [Out, MarshalAs(UnmanagedType.AnsiBStr)] string buf, uint buflen);
        int compat_filterHasReset(uint filterID);
        int compat_filterHasResetEx(uint filterID, uint filterPageId);
        int compat_shortOSDmessage([In, MarshalAs(UnmanagedType.AnsiBStr)] string msg, uint duration); //duration is in frames
        int compat_shortOSDmessageAbsolute([In, MarshalAs(UnmanagedType.AnsiBStr)] string msg, uint duration, uint posX, uint posY); //duration is in frames
