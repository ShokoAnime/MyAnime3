...
namespace MyAnimePlugin3.Downloads
{
	public class TorrentsAnimeBytes : ITorrentSource
	{
		...
		public List<TorrentLink> GetTorrents(List<string> searchParms)
		{
			try
			{
				if (string.IsNullOrEmpty(BaseConfig.Settings.AnimeBytesUsername) || string.IsNullOrEmpty(BaseConfig.Settings.AnimeBytesPassword))
					return new List<TorrentLink>();

				if (string.IsNullOrEmpty(BaseConfig.Settings.AnimeBytesCookieHeader))
				{
					string cookie = Login(BaseConfig.Settings.AnimeBytesUsername, BaseConfig.Settings.AnimeBytesPassword);
					BaseConfig.Settings.AnimeBytesCookieHeader = cookie;
				}
				...
				string output = Utils.DownloadWebPage(url, BaseConfig.Settings.AnimeBytesCookieHeader, true);
