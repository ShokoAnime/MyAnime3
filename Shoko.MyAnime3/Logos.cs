...
namespace Shoko.MyAnime3
{
	public class Logos
	{
		...
		*/
        ...
        public static string buildLogoImage(VM_AnimeEpisode_User ep)
		{
			...
			int bestWidth = 0;
			int bestHeight = 0;
			VM_VideoDetailed bestFile = null;
			...
			if (bestFile.GetVideoCodec().ToUpper().Contains("H264") && File.Exists(SkinSettings.LogoCodecAVC))
                logosForBuilding.Add(SkinSettings.LogoCodecAVC);
			if (bestFile.GetVideoCodec().ToUpper().Contains("DIVX") && File.Exists(SkinSettings.LogoCodecDivx))
                logosForBuilding.Add(SkinSettings.LogoCodecDivx);
			if (bestFile.GetVideoCodec().ToUpper().Contains("XVID") && File.Exists(SkinSettings.LogoCodecDivx))
                logosForBuilding.Add(SkinSettings.LogoCodecDivx);
