	...
	public class Utils
	{
	    ...
	    public static void PromptToRateSeriesOnCompletion(VM_AnimeSeries_User ser)
	    {
	        try
	        {
	            ...
	            if (!ser.Anime.FinishedAiring || ser.UnwatchedEpisodeCount > 0) return;
        ...
        public static void PromptToRateSeriesMaually(VM_AnimeSeries_User ser)
        {
            try
            {
                // if we have no anidb info return
		...
		public static string PromptSelectTag(string title)
		{
			...
			allTags = new List<string>(VM_ShokoServer.Instance.ShokoServices.GetAllTagNames());
			allTags.Sort();
